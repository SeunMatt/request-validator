...
public class RequiredIfRuleValidator implements RuleValidator {
    ...
    public ValidationResult isValid(Object value, Rule rule) {
        ...
        if (actualConditionalFieldValue == null) {
            ...
            isRequired = false;
        } else if (conditionalFieldValue != null && !conditionalFieldValue.isEmpty() && !actualConditionalFieldValue.equals(
                conditionalFieldValue)) {
            //if the request contains the conditional field but its value is not equal to the conditionValue
        ...
        if (isRequired) {
            return ValidationHelper.isValidRequired(value) ? ValidationResult.success() :
                   ValidationResult.failed(
                           "The " + rule.getKey() + " field is required when if " + String.join(" = ", parameters));
        }
